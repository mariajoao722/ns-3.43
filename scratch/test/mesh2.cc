...
std::set<uint32_t> multicastGroupNodes = {1};
...
MeshTest::CreateNodes()
{
    ...
    positionAlloc->Add(Vector(50.0, 0.0, 0.0));   // Node 0
    positionAlloc->Add(Vector(50.0, 50.0, 0.0));  // Node 1
    positionAlloc->Add(Vector(28, 95, 0.0));      // Node 2
    positionAlloc->Add(Vector(72, 95, 0.0));      // Node 3
    positionAlloc->Add(Vector(50.0, 140.0, 0.0)); // Node 4
    ...
    if (m_pcap)
    {
        wifiPhy.EnablePcapAll(
            std::string("/home/mpais/ns-allinone-3.43/ns-3.43/scratch/meshtrace2/mp"));
    }
    if (m_ascii)
    {
...
MeshTest::Run()
{
    ...
    AnimationInterface anim("mesh2.xml");

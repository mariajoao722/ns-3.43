...
std::set<uint32_t> multicastGroupNodes = {1, 9};
...
MeshTest::CreateNodes()
{
    ...
    if (m_pcap)
    {
        wifiPhy.EnablePcapAll(
            std::string("/home/mpais/ns-allinone-3.43/ns-3.43/scratch/meshtrace1/mp"));
    }
    if (m_ascii)
    {
...
static void
OnHwmpRouteChange(const ns3::dot11s::RouteChange& rc)
{
  std::cout << "[HWMP] " << rc.type
            << " dst=" << rc.destination
            << " via=" << rc.retransmitter
            << " if="  << rc.interface
            << " metric=" << rc.metric
            << " life=" << rc.lifetime.GetSeconds() << "s"
            << " seq=" << rc.seqnum
            << std::endl;
}
...
MeshTest::Run()
{
    ...
    InstallApplication();
    InstallUnicastTraffic(11, 3, 9001, 512, "5Mbps", 10.0, m_totalTime - 2.0);

    ...
    if (outInterface->IsUp())
    {
        ...
        else
        {
            CallTxTrace(ipHeader, packet, this, interface);
                 ...
                 Ipv4Address ipAddr = ipHeader.GetSource();
        uint32_t ipRaw = ipAddr.Get();

        uint8_t ip1 = (ipRaw >> 24) & 0xFF;
        uint8_t ip2 = (ipRaw >> 16) & 0xFF;
        uint8_t ip3 = (ipRaw >> 8) & 0xFF;
        uint8_t ip4 = ipRaw & 0xFF;
        std::string ipStr1 = std::to_string(ip1) + "." + std::to_string(ip2) + "." +
                                   ...
                                   const Ipv4Header& header)
{
    ...
    // tirar includes que acrescentei
            Ipv4Address ipAddr = ipHeader.GetSource();
        uint32_t ipRaw = ipAddr.Get();
        ...
        uint8_t ip3 = (ipRaw >> 8) & 0xFF;
        uint8_t ip4 = ipRaw & 0xFF;
        std::string ipStr = std::to_string(ip1) + "." + std::to_string(ip2) + "." +
                    std::to_string(ip3) + "." + std::to_string(ip4);
